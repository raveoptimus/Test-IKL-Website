  ...
  return (
    <div className="space-y-12 animate-fade-in pb-24">
       ...
       <div className="text-center py-10 relative">
          <h1 className="text-5xl md:text-9xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-600 mb-4 tracking-tighter drop-shadow-xl">
             STATS CENTER
          </h1>
          <div className="w-24 md:w-32 h-1 md:h-1.5 bg-gradient-to-r from-transparent via-ikl-red to-transparent mx-auto"></div>
       </div>
       ...
       <div className="flex justify-center mb-10 px-4">
          <div className="w-full md:w-auto flex flex-row gap-2 bg-black/40 border border-white/10 rounded-lg p-2 backdrop-blur-md overflow-x-auto">
             <button
                ...
                className={`flex-1 md:flex-none px-6 py-3 md:px-10 md:py-4 rounded-md font-display text-xl md:text-2xl tracking-widest transition-all duration-300 whitespace-nowrap ${
                    activeTab === 'roster' 
       ...
       {activeTab === 'roster' && (
         <div className="bg-ikl-panel rounded-xl border border-white/10 overflow-hidden shadow-2xl animate-slide-up mx-2 md:mx-0">
            <div className="p-4 md:p-8 bg-gradient-to-r from-black via-[#111] to-black border-b border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
               <h3 className="text-2xl md:text-4xl font-display text-white tracking-wider text-center md:text-left">LEAGUE PLAYER DATABASE</h3>
               <div className="text-xs md:text-sm text-gray-500 font-mono uppercase tracking-widest">Tap Headers to Sort</div>
            </div>
            
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[900px]">
                 <thead>
                    <tr className="bg-black/50 text-gray-400 uppercase text-lg font-bold tracking-widest border-b border-white/10">
                       ...
                       <th className="p-5 text-right cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('stats.death')}>
                           Deaths <SortIcon columnKey="stats.death" />
                       </th>
                 ...
                 <tbody className="divide-y divide-white/5">
                    {sortedPlayers.map((p, idx) => {
                       ...
                       // Format Nickname (Remove Team Prefix)
                       const nickname = p.name.includes('.') ? p.name.split('.').pop() : p.name;
                       
                       return (
                       <tr key={p.id} className={`hover:bg-white/5 transition-colors group ${idx % 2 === 0 ? 'bg-white/[0.02]' : 'bg-transparent'}`}>
                          <td className="p-5 font-display text-4xl font-bold text-white group-hover:text-ikl-red transition-colors uppercase tracking-wide">{nickname}</td>
                          <td className="p-5 text-gray-300 text-xl font-bold">
                              <div className="flex items-center gap-4">
                                  {teamLogo && (
                                      <img src={teamLogo} alt={p.team} className="w-12 h-12 object-contain" referrerPolicy="no-referrer" />
                                  )}
                                  <span className="font-display text-2xl tracking-wide">{p.teamAbv || p.team}</span>
                              </div>
                          </td>
                          <td className="p-5"><span className="px-4 py-1.5 bg-white/10 border border-white/5 rounded text-sm uppercase text-white font-bold tracking-widest">{ROLE_LABELS[p.role]}</span></td>
                          <td className="p-5 text-right font-display text-3xl font-bold text-gray-500">{p.stats.matches}</td>
                          <td className="p-5 text-right font-display text-3xl font-bold text-gray-300">{p.stats.kill}</td>
                          <td className="p-5 text-right font-display text-3xl font-bold text-red-400">{p.stats.death}</td>
                          <td className="p-5 text-right font-display text-3xl font-bold text-gray-300">{p.stats.assist}</td>
                          <td className="p-5 text-right font-display text-4xl font-bold text-white">
                              <span className={p.calcKDA > 5 ? 'text-ikl-gold' : p.calcKDA > 3 ? 'text-ikl-red' : 'text-gray-300'}>
                                {p.calcKDA.toFixed(2)}
                              </span>
                          </td>
                          <td className="p-5 text-right font-display text-3xl text-ikl-gold font-bold">{p.stats.gpm}</td>
                       </tr>
                    )})}
                 </tbody>
              </table>
       ...
       {activeTab === 'highlights' && (
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-slide-up px-2">
            ...
            {/* Top KDA Chart */}
            <div className="bg-gradient-to-br from-[#111] to-black p-4 md:p-8 rounded-xl border border-white/10 hover:border-ikl-red/30 transition-all group shadow-2xl relative overflow-hidden">
               <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  <svg className="w-24 h-24 md:w-40 md:h-40 text-ikl-red" fill="currentColor" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
               </div>
               <h3 className="text-3xl md:text-4xl font-display text-white mb-1 tracking-wider relative z-10">HIGHEST <span className="text-ikl-red">KDA</span> RATIO</h3>
               ...
               <div className="h-[300px] md:h-[350px] relative z-10">
                 <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={topKDA} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                      ...
                      <YAxis dataKey="name" type="category" stroke="#fff" width={80} tick={{fontFamily: 'Teko', fontSize: 16, letterSpacing: '1px'}} />
                      <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,42,42,0.1)'}} />
                      <Bar dataKey="calcKDA" barSize={20} radius={[0, 4, 4, 0]}>
            ...
            {/* Top GPM Chart */}
            <div className="bg-gradient-to-br from-[#111] to-black p-4 md:p-8 rounded-xl border border-white/10 hover:border-ikl-gold/30 transition-all group shadow-2xl relative overflow-hidden">
               <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  <svg className="w-24 h-24 md:w-40 md:h-40 text-ikl-gold" fill="currentColor" viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
               </div>
               <h3 className="text-3xl md:text-4xl font-display text-white mb-1 tracking-wider relative z-10">HIGHEST <span className="text-ikl-gold">GPM</span></h3>
               ...
               <div className="h-[300px] md:h-[350px] relative z-10">
                 <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={topGPM} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <XAxis dataKey="name" stroke="#fff" tick={{fontFamily: 'Teko', fontSize: 16, letterSpacing: '1px'}} />
            ...
            <div className="lg:col-span-2 bg-gradient-to-br from-[#111] to-black p-4 md:p-8 rounded-xl border border-white/10 hover:border-blue-500/30 transition-all group shadow-2xl relative overflow-hidden">
               <h3 className="text-3xl md:text-4xl font-display text-white mb-1 tracking-wider relative z-10">MOST <span className="text-blue-500">KILLS</span> (TOTAL)</h3>
               <div className="h-[250px] md:h-[300px] relative z-10 mt-8">
                 <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={topKills} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#222" vertical={false} />
                      <XAxis dataKey="name" stroke="#fff" tick={{fontFamily: 'Teko', fontSize: 16}} />
                      <YAxis stroke="#555" />
